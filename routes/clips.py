...
@router.get("/clips", response_model=ClipListResponse, summary="Отримати список нарізок/кліпів з бази Vamous (MongoDB)")
async def list_clips(
    ...
    """
    Повертає кліпи напряму з вашої MongoDB бази Vamous (35 700+ кліпів / ~580 за 1 день):
...
@router.post("/clips", response_model=ClipItem, status_code=status.HTTP_201_CREATED, summary="Додати новий кліп")
...
@router.post("/refresh-database", response_model=RefreshResponse, summary="Статус синхронізації бази")
async def refresh_database():
    return RefreshResponse(
        status="success",
        message="База MongoDB twitch_data підключена та актуальна"
    )
